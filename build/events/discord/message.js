...
class Message extends Event_1.default {
    ...
    async run(message) {
        ...
        if (!this.client.cooldown.has(command.name)) {
            this.client.cooldown.set(command.name, new discord_js_1.default.Collection());
        }
        const now = Date.now();
        ...
        if (command.authorPermission) {
            ...
            if (neededPerms.length)
                return message.channel.send(`❌ | You don't have enough permissions. You need \`${neededPerms.join('`, `')}\``);
        }
            ...
            if (neededPerms.length)
                return message.channel.send(`❌ | I don't have enough permissions. I need \`${neededPerms.join('`, `')}\``);
        }
        if (command.devOnly && command.devOnly === true) {
            if (!['561866357218607114', '690822196972486656'].includes(message.author.id))
                return message.channel.send(`❌ | This command can be used by the developers only.`);
        }
